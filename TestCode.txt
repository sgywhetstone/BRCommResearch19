...
#Define a function that turns an input word on {A,B} into the appropriate composition of psi and phi
#MapToComm := function(word)
#	local C, GenC, CommHom;
#	C := GroupByGenerators([psi, psi2,phi, phi2]);
#	GenC := GeneratorsOfGroup(C);
#	CommHom := GroupHomomorphismByImage(f, C, Genf, GenC);
#	return CommHom(word);
#end;

#FIXME Same as above - Try by converting the word (which will be a Comm of something, and so a group object)
#into a string? and then recursively stripping off the last "element"
#MapToComm:= function(word)
#	local;
